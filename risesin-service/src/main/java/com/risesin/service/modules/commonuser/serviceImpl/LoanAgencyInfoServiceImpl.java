...
import com.risesin.service_api.dao.comuser.LoanAgencyInfoDao;
...
/**
 * LoanAgencyInfo的服务接口的实现类
 *
 * @author honey
 */
@Service
public class LoanAgencyInfoServiceImpl extends SingletonParent implements BaseInterface {
    private final Logger LOG = LogManager.getLogger(this.getClass());

    @Autowired
    private LoanAgencyInfoDao loanAgencyInfoDao;
     ...
     * @return
     */
    public List<LoanAgencyInfo> findAll() {
        return loanAgencyInfoDao.findAll();
    }
     ...
     * @return
     */
    public Page<LoanAgencyInfo> findSearch(Map whereMap, int page, int size) {
        Specification<LoanAgencyInfo> specification = createSpecification(whereMap);
        PageRequest pageRequest = PageRequest.of(page - 1, size);
        return loanAgencyInfoDao.findAll(specification, pageRequest);
    }
     ...
     * @return
     */
    public List<LoanAgencyInfo> findSearch(Map whereMap) {
        Specification<LoanAgencyInfo> specification = createSpecification(whereMap);
        return loanAgencyInfoDao.findAll(specification);
    }
     ...
     * @return
     */
    public LoanAgencyInfo findById(Long id) {
        return loanAgencyInfoDao.findById(id).get();
    }
     ...
     * @param value
     */
    public void add(LoanAgencyInfo value) {
        loanAgencyInfoDao.save(value);
    }
     ...
     * @param value
     */
    public void update(LoanAgencyInfo value) {
        loanAgencyInfoDao.save(value);
    }
    ...
    public void deleteById(Long id) {
        loanAgencyInfoDao.deleteById(id);
    }
     ...
     * @return
     */
    private Specification<LoanAgencyInfo> createSpecification(Map searchMap) {

        return new Specification<LoanAgencyInfo>() {

            @Override
            public Predicate toPredicate(Root<LoanAgencyInfo> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                List
