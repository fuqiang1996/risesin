...
import com.risesin.service_api.dao.comuser.CommonUserDao;
 ...
 */
@Service
public class CommonUserServiceImpl extends SingletonParent implements BaseInterface {
     ...
     * @return
     */
    public Page<CommonUser> findSearch(Map whereMap, int page, int size) {
        Specification<CommonUser> specification = createSpecification(whereMap);
        PageRequest pageRequest = PageRequest.of(page - 1, size);
        return commonUserDao.findAll(specification, pageRequest);
    }
     ...
     * @return
     */
    public List<CommonUser> findSearch(Map whereMap) {
        Specification<CommonUser> specification = createSpecification(whereMap);
        return commonUserDao.findAll(specification);
    }
    ...
    public CommonUser findById(Long id) {
        return commonUserDao.findById(id).get();
    }
     ...
     * @param value
     */
    public void add(CommonUser value) {
        commonUserDao.save(value);
    }
     ...
     * @param value
     */
    public void update(CommonUser value) {
        commonUserDao.save(value);
    }
     ...
     * @return
     */
    private Specification<CommonUser> createSpecification(Map searchMap) {

        return new Specification<CommonUser>() {

            @Override
            public Predicate toPredicate(Root<CommonUser> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                List
