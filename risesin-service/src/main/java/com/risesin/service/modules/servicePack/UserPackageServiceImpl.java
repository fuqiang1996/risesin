package com.risesin.service.modules.servicePack;
...
import com.risesin.service_api.dao.servicePack.UserPackageDao;
...
/**
 * UserPackage的服务接口的实现类
 *
 * @author honey
 */
@Service
public class UserPackageServiceImpl extends SingletonParent implements BaseInterface<UserPackage> {
     ...
     * @return
     */
    public Page<UserPackage> findSearch(Map whereMap, int page, int size) {
        Specification<UserPackage> specification = createSpecification(whereMap);
        PageRequest pageRequest = PageRequest.of(page - 1, size);
        return userPackageDao.findAll(specification, pageRequest);
    }
     ...
     * @return
     */
    public List<UserPackage> findSearch(Map whereMap) {
        Specification<UserPackage> specification = createSpecification(whereMap);
        return userPackageDao.findAll(specification);
    }
    ...
    public UserPackage findById(Long id) {
        return userPackageDao.findById(id).get();
    }
     ...
     * @param value
     */
    public void add(UserPackage value) {
        userPackageDao.save(value);
    }
     ...
     * @param value
     */
    public void update(UserPackage value) {
        userPackageDao.save(value);
    }
     ...
     * @return
     */
    private Specification<UserPackage> createSpecification(Map searchMap) {

        return new Specification<UserPackage>() {

            @Override
            public Predicate toPredicate(Root<UserPackage> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                List
