...
import com.risesin.service_api.dao.comuser.ChannelDefinitionDao;
...
/**
 * ChannelDefinition的服务接口的实现类
 *
 * @author honey
 */
@Service
public class ChannelDefinitionServiceImpl extends SingletonParent implements BaseInterface {
    private final Logger LOG = LogManager.getLogger(this.getClass());

    @Autowired
    private ChannelDefinitionDao channelDefinitionDao;
     ...
     * @return
     */
    public List<ChannelDefinition> findAll() {
        return channelDefinitionDao.findAll();
    }
     ...
     * @return
     */
    public Page<ChannelDefinition> findSearch(Map whereMap, int page, int size) {
        Specification<ChannelDefinition> specification = createSpecification(whereMap);
        PageRequest pageRequest = PageRequest.of(page - 1, size);
        return channelDefinitionDao.findAll(specification, pageRequest);
    }
     ...
     * @return
     */
    public List<ChannelDefinition> findSearch(Map whereMap) {
        Specification<ChannelDefinition> specification = createSpecification(whereMap);
        return channelDefinitionDao.findAll(specification);
    }
     ...
     * @return
     */
    public ChannelDefinition findById(Long id) {
        return channelDefinitionDao.findById(id).get();
    }
     ...
     * @param value
     */
    public void add(ChannelDefinition value) {
        channelDefinitionDao.save(value);
    }
     ...
     * @param value
     */
    public void update(ChannelDefinition value) {
        channelDefinitionDao.save(value);
    }
    ...
    public void deleteById(Long id) {
        channelDefinitionDao.deleteById(id);
    }
     ...
     * @return
     */
    private Specification<ChannelDefinition> createSpecification(Map searchMap) {

        return new Specification<ChannelDefinition>() {

            @Override
            public Predicate toPredicate(Root<ChannelDefinition> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                List
