...
import com.risesin.service_api.dao.sysuser.SysCompanyDao;
 ...
 */
@Service
public class SysCompanyServiceImpl extends SingletonParent implements BaseInterface<SysCompany> {
     ...
     * @return
     */
    public Page<SysCompany> findSearch(Map whereMap, int page, int size) {
        Specification<SysCompany> specification = createSpecification(whereMap);
        PageRequest pageRequest = PageRequest.of(page - 1, size);
        return sysCompanyDao.findAll(specification, pageRequest);
    }
     ...
     * @return
     */
    public List<SysCompany> findSearch(Map whereMap) {
        Specification<SysCompany> specification = createSpecification(whereMap);
        return sysCompanyDao.findAll(specification);
    }
     ...
     * @return
     */
    public SysCompany findById(Long id) {
        return sysCompanyDao.findById(id).get();
    }
     ...
     * @param value
     */
    public void add(SysCompany value) {
        sysCompanyDao.save(value);
    }
     ...
     * @param value
     */
    public void update(SysCompany value) {
        sysCompanyDao.save(value);
    }
     ...
     * @return
     */
    private Specification<SysCompany> createSpecification(Map searchMap) {

        return new Specification<SysCompany>() {

            @Override
            public Predicate toPredicate(Root<SysCompany> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                List
